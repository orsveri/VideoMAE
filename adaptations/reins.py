"""
From official Rein implementation: https://github.com/w1oves/Rein
"""
import torch
...
class Reins(nn.Module):
    def __init__(
        ...
        self.create_model()
        # fuse methods for classification
        if fuse_method == "simple_concat":
            self.fuse = self._fuse_simple_concat
        elif fuse_method == "cross_attention":
            self.fuse = self.SimpleCrossAttnFusion(embed_dim=embed_dims, num_heads=4, mlp_ratio=4.0)
        else:
            raise ValueError(f"Unknown fuse_method. Given: {fuse_method}, possible methods: simple_concat, cross_attention")
        print(f"==== Rein: link token: {link_token_to_query}, fuse method: {fuse_method}")
    ...
    def return_auto(self, feats):
        if self.link_token_to_query:
            ...
            querys = querys.unsqueeze(0).expand(B, -1, -1)
            feats = self.fuse(feats, querys)
            return feats
        else:
            return feats
        
    def _fuse_simple_concat(self, feats, tokens):
        return torch.cat([feats, tokens], dim=1)
...
class SimpleCrossAttnFusion(nn.Module):
    """
    A one-layer cross-attention block that fuses Rein's final queries with the final features.
    - queries: (B, Q, D)
    - feats:   (B, T, D)
    returns   (B, Q, D)
    
    This is essentially queries attending to feats, so queries get updated 
    based on information from feats.
    """
    def __init__(self, embed_dim, num_heads=4, mlp_ratio=4.0):
        ...
        # A small MLP for a bit more capacity (typical in transformer blocks)
    ...
    def forward(self, feats, queries):
        # queries -> Q, feats -> K,V for cross-attn
        # queries: (B, Q, D), feats: (B, T, D)
        
        # 1) Cross-Attention
        attended, _ = self.attn(
            query=queries,         # shape (B, Q, D)
            key=feats,             # shape (B, T, D)
            value=feats            # shape (B, T, D)
        )
        # Add & Norm
        x = queries + attended
        x = self.ln1(x)
