...
class ReinVisionTransformer(nn.Module):
    ...
    def __init__(self, 
                 ...
                 rein_link_token_to_query=False,
                 rein_token_length=100,
                 rein_fuse_method: str = "simple_concat"):
        super().__init__()
        ...
        self.reins = LoRAReins(
            num_layers=depth, embed_dims=embed_dim, patch_size=patch_size, 
            link_token_to_query=rein_link_token_to_query, query_dims=embed_dim, 
            token_length=rein_token_length, fuse_method=rein_fuse_method
            )
    ...
    def forward_features(self, x):
        ...
        if self.use_checkpoint:
            for idx, blk in enumerate(self.blocks):
                ...
                x = self.reins.forward(x, idx, batch_first=True, has_cls_token=False)
        else:   
            for idx, blk in enumerate(self.blocks):
                x = blk(x)
                # Set batch_first to True if the shape of x is (B, N, C), else False
    ...
    def train(self, mode: bool = True):
        ...
        set_requires_grad(self, ["reins", "head"])
        set_train(self, ["reins", "head"])


@register_model
def rein_vit_small_patch16_224(pretrained=False, **kwargs):
