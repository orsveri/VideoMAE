...
from engine_for_pretraining import train_one_epoch_triple
...
def get_args():
    ...
    parser.add_argument('--batch_size3', default=64, type=int)
    ...
    parser.add_argument('--data_path2', default='/path/to/list_kinetics-400', type=str,
                        help='dataset path')
    parser.add_argument('--data_path3', default='/path/to/list_kinetics-400', type=str,
                        help='dataset path')
    parser.add_argument('--view_fps', type=int, default=10)
    parser.add_argument('--data_set1', default='Kinetics-400',
                        choices=['Kinetics-400', 'SSV2', 'UCF101', 'HMDB51', 'DoTA', 'DADA2K', 'CAP-DATA', 'BDD100K', 'SHIFT', 'image_folder'],
                        type=str, help='dataset')
    parser.add_argument('--data_set2', default='Kinetics-400',
                        choices=['Kinetics-400', 'SSV2', 'UCF101', 'HMDB51', 'DoTA', 'DADA2K', 'CAP-DATA', 'BDD100K', 'SHIFT', 'image_folder'],
                        type=str, help='dataset')
    parser.add_argument('--data_set3', default='Kinetics-400',
                        choices=['Kinetics-400', 'SSV2', 'UCF101', 'HMDB51', 'DoTA', 'DADA2K', 'CAP-DATA', 'BDD100K', 'SHIFT', 'image_folder'],
                        type=str, help='dataset')
    ...
    parser.add_argument('--sampling_rate3', type=int, default= 4)
...
def main(args):
    ...
    args3.data_set = args.data_set3
    args3.data_path = args.data_path3
    args3.sampling_rate = args.sampling_rate3
    ...
    dataset_train3 = build_pretraining_dataset(is_train=True, args=args3)

    print("Datasets are ready, full len: ")
    print(f"\t - 1: {args.data_set1} {len(dataset_train1)}")
    print(f"\t - 2: {args.data_set2} {len(dataset_train2)}")
    print(f"\t - 3: {args.data_set3} {len(dataset_train3)}")

    num_tasks = utils.get_world_size()
    global_rank = utils.get_rank()
    sampler_rank = global_rank

    total_batch_size = (args.batch_size1 + args.batch_size2 + args.batch_size3) * num_tasks
    num_batches = int(round(args.nb_samples_per_epoch / (args.batch_size1 + args.batch_size2 + args.batch_size3)))
    nb_samples_per_epoch1 = args.batch_size1 * num_batches
    nb_samples_per_epoch2 = args.batch_size2 * num_batches
    nb_samples_per_epoch3 = args.batch_size3 * num_batches
    #
    ...
    #
    if args.nb_samples_per_epoch and (nb_samples_per_epoch3 < len(dataset_train3)):
        sampler_train3 = utils.ShortDistributedSampler(
            dataset_train3, num_replicas=num_tasks, rank=sampler_rank, shuffle=True,
            num_samples_per_epoch=nb_samples_per_epoch3
        )
    else:
    ...
    #
    num_training_steps_per_epoch = (sampler_train1.total_size + sampler_train2.total_size + sampler_train3.total_size) // total_batch_size
    
    print("Sampler_train = ")
    print(f"\t- 1: {str(sampler_train1)}, len: {sampler_train1.total_size}")
    print(f"\t- 2: {str(sampler_train2)}, len: {sampler_train2.total_size}")
    print(f"\t- 3: {str(sampler_train3)}, len: {sampler_train3.total_size}")
    ...
    data_loader_train3 = torch.utils.data.DataLoader(
        dataset_train3, sampler=sampler_train3,
        batch_size=args.batch_size3,  # Half-batch for dataset 2
        num_workers=args.num_workers // 4,
        pin_memory=args.pin_mem,
        drop_last=True,
        worker_init_fn=utils.seed_worker,
    ...
    dl_lengths = len(data_loader_train1), len(data_loader_train2), len(data_loader_train3)
    if len(set(dl_lengths)) > 1:
        max_len_id = dl_lengths.index(max(dl_lengths))
        smaller_dl_ids = [0, 1, 2]
        smaller_dl_ids.pop(max_len_id)
        larger_loader = [data_loader_train1, data_loader_train2, data_loader_train3][max_len_id]
        smaller_loader1 = [data_loader_train1, data_loader_train2, data_loader_train3][smaller_dl_ids[0]]
        smaller_loader2 = [data_loader_train1, data_loader_train2, data_loader_train3][smaller_dl_ids[1]]
        # Cycle through the smaller DataLoader
        smaller_loader1 = CyclicDataLoader(smaller_loader1)
        smaller_loader2 = CyclicDataLoader(smaller_loader2)
    else:
        larger_loader, smaller_loader1, smaller_loader2 = data_loader_train1, data_loader_train2, data_loader_train3
    ...
    if len(data_loader_train1) != len(data_loader_train3):
        print(f"WARNING len(data_loader_train1) != len(data_loader_train3): {len(data_loader_train1)} != {len(data_loader_train3)}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            ...
            if isinstance(smaller_loader1, CyclicDataLoader):
                smaller_loader1.set_epoch(epoch)
            else:
                smaller_loader1.sampler.set_epoch(epoch)
            if isinstance(smaller_loader2, CyclicDataLoader):
                smaller_loader2.set_epoch(epoch)
            else:
                smaller_loader2.sampler.set_epoch(epoch)
        if log_writer is not None:
            log_writer.set_step(epoch * num_training_steps_per_epoch)
        train_stats, grad_norms = train_one_epoch_triple(
            model, larger_loader, smaller_loader1, smaller_loader2,
