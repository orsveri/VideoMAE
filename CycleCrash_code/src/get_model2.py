...
from CycleCrash_code.src.vidnext.vidnext import Model as VidNeXt
from CycleCrash_code.src.vidnext.convnextplusvanilla import Model as ConvNeXtVanillaTransformer
from CycleCrash_code.src.vidnext.rensetplusNST import Model as ResNetNSTransformer
#from CycleCrash_code.src.timesformer.models.vit import TimeSformer
#from CycleCrash_code.src.vivit_pytorch.video_transformer import ViViT
import torch
...
class VideoEncoder(nn.Module):
    def __init__(self, model_type='VidNeXt', task_type='binclass', num_classes=5, segment_length_frames=16):
        super().__init__() 
        embed_dims = None
        if model_type == 'VidNeXt':
            self.vid_trans = VidNeXt(segment_length=segment_length_frames)
            embed_dims=1024
        if model_type == 'ConvNeXtVanillaTransformer':
            self.vid_trans = ConvNeXtVanillaTransformer(segment_length=segment_length_frames)
            embed_dims=1024
        if model_type == 'ResNetNSTtransformer':
            self.vid_trans = ResNetNSTransformer(segment_length=segment_length_frames)
            embed_dims = 512
        if model_type == 'TimeSformer':
            print("No TimeSformer!")
