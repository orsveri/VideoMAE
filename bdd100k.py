...
if __name__ == "__main__":
    ...
    if False:
        ...
        out_path = "/gpfs/work3/0/tese0625/datasets/bdd100k_splits/all.txt"
        filelist = []
        if False:
    ...
    L2 = len(dataloader)
    print_break = L2 // 2
    print(f"\nDataset length: {L}, Batch size: 100, Batch numbers: {L2}, print break every {print_break} batches\n")
    ...
    for idx, batch in tqdm(enumerate(dataloader), total=L2, desc="Validating dataset"):
        ...
        if idx % print_break == 0:
            ...
            problems_ = "\n".join(problems)
            if len(problems) > 0:
                with open(f"/gpfs/work3/0/tese0625/datasets/bdd100k_splits/decord_err_train_b200_{idx}.txt", mode="w") as f:
                    f.write(problems_)
            print(f"NB of corrupted videos found so far (decord): {len(problems)}")
            problems = dataloader.dataset.corrupt_clips
            problems_ = "\n".join(problems)
            if len(problems) > 0:
                with open(f"/gpfs/work3/0/tese0625/datasets/bdd100k_splits/opencv_err_train_b200_{idx}.txt", mode="w") as f:
                    f.write(problems_)
    ...
    problems_ = "\n".join(problems)
    if len(problems) > 0:
        with open(f"/gpfs/work3/0/tese0625/datasets/bdd100k_splits/decord_err_train_b200.txt", mode="w") as f:
            f.write(problems_)
    problems = dataloader.dataset.corrupt_clips
    problems_ = "\n".join(problems)
    if len(problems) > 0:
        with open(f"/gpfs/work3/0/tese0625/datasets/bdd100k_splits/opencv_err_train_b200.txt", mode="w") as f:
            f.write(problems_)
